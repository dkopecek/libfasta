...
#ifndef __XCONCAT
# define __XCONCAT1(a,b) a ## b
# define __XCONCAT(a,b) __XCONCAT1(a,b)
#endif
...
#define __assume(expr, exprstr, retval, ...)                            \
        do {                                                            \
                int __XCONCAT(__cont, __LINE__) = 1;                        \
                if (!(expr)) {                                          \
                        __emitmsg ("%s:%d (%s): Assumption `%s' not fulfilled!\n", __FILE__, __LINE__, __PRETTY_FUNCTION__, exprstr); \
                        do {__LB(__VA_ARGS__)} while((__XCONCAT(__cont, __LINE__) = 0)); \
                        if (__XCONCAT(__cont, __LINE__) == 0) __terminate(retval); \
                } else {                                                \
